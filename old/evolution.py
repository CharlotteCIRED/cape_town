...
def scenarios(t, etat_initial_erreur, etat_initial_people_housing_type, etat_initial_people_center, etat_initial_people1, etat_initial_hous1, etat_initial_housing1, etat_initial_rent1, etat_initial_utility, etat_initial_revenu_in, trans, grid, land, job, param, macro_data, option):
    """ Simulations.
    ...
    t_calc = np.arange(t[0], t[len(t)-1] + 1, (t[1]-t[0])/iter_calc_lite)
    option["ajust_bati"] = 0
    ...
    # %% Iteration: find the equilibrium for each year
    ...
    for index in range(0, len(t_calc)):
        ...
        if index > 0:
            ...
            ### FIRST SIMULATION - Housing can be freely adjusted
            # Output: target of private developpers
            ...
            deriv_housing_temp = evolution_housing(grid, land, trans, param, macro_data, option, job, t_calc[index - 1], simul_without_constraint_rent1[0, :], simul_without_constraint_rent1[0, :], etat_tmp_housing1[0, :], etat_tmp_people_housing_type[0, :] * coeff_land[0, :] + etat_tmp_people_housing_type[3, :] * coeff_land[3, :])
...
def evolution_housing(grid, land, trans, param, macro_data, option, job, t, rentA, rentB, housing1, people_formal1):
    ...
    housing_target = housing_target_b * 0.5 + housing_target_a * 0.5 #Weighting between the case with and without anticipations

    housing_target = np.minimum(housing_target, housing_limite_ici)

    sortie_housing = 1 / param["time_invest_h"] * (housing_target - housing1) * ((housing_target - housing1) > 0) + param["depreciation_rate"] * (-housing1) * ((housing_target - housing1) <= 0) #Derivation with inertia
