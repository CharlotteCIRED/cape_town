...
class TransportData:
    ...
    def charges_temps_polycentrique_CAPE_TOWN_3(self, option,grille,macro,param,poly,t_trafic):
        ...
        trans_reliable = np.ones(0, len(grille.dist))
        ...
        prix_metro_2012_km = 1.5 / 40 * macro.spline_inflation(2012 - param["year_begin"]) / macro.spline_inflation(2015 - param["year_begin"])  #0.164
        prix_metro_2012_fixe_mois = 121.98 * macro.spline_inflation(2012 - param["year_begin"]) / macro.spline_inflation(2015 - param["year_begin"]) #4.48*40
        ...
        inflation = macro.spline_inflation(t_trafic)
        infla_2012 = macro.spline_inflation(2012 - param["year_begin"])
        ...
        prix_essence = macro.spline_carburant(t_trafic)
        ...
        mult_prix_sortie(:,:,1) = np.zeros((temps_sortie(:,:,1)).shape)
        ...
        prix_sortie_unitaire(:,1) = np.ones(prix_essence_mois.shape)
        ...
        prix_monetaire = np.zeros(len(poly.code_emploi_poly[referencement]), size(temps_sortie, 2), trans_nbre_modes)
        cout_generalise = (np.zeros(len(poly.code_emploi_poly[referencement]), size(temps_sortie, 2), len(t_trafic)))
        quel = (np.zeros(size(poly.code_emploi_poly[referencement], 1), size(temps_sortie,2)))

        mult = cout_generalise
        cout_generalise_ancien = cout_generalise
        tbis = t_trafic

        taille_menage_mat = np.matlib.repmat(param["taille_menage_transport"], 1, len(poly.code_emploi_init) / param["multiple_class"])
